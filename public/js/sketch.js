...
function makeBestMove(color) {
    ...
    game.move(move);
    //make ai move here
    let movesMade=game.history({ verbose: true })
    let moveToMake = movesMade[movesMade.length - 1]
    movePiece(moveToMake.from)
    movePiece(moveToMake.to)
...
var undo_stack = [];
function registerPlayerMove(source, target,promotion='q') {
  makingAIMove=true
    undo_stack = [];
...
class Piece {
  ...
  swapPiece(newName,pieceImg){
    ...
    let box=null
    let piece_type= name.toLowerCase().charAt(0)
    let piece_color= this.color.charAt(0)
    for (let i = 0; i < squares.length; i++) {
      if (squares[i].clicked(this.x, this.y) ) {
        
     box=squares[i].name
      }
    }
    
    game.remove(box) // remove  swapped out piece from box
    game.put({ type:piece_type, color:piece_color }, box) // put a swapped in piece on box
  ...
  move(px, py,flags="n") {
    ...
    if(gameState.move.flags){
      if(gameState.move.flags === "c"){
        game.remove(gameState.move.to)
      }
    }
    registerPlayerMove(gameState.move.from, gameState.move.to)
