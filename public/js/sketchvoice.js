...
var boxes = [ "A1", "B1", "C1", "D1", "E1", "F1", "G1", "H1",
...
var boxes_names = [ "11", "21", "31", "41", "51", "61", "71", "81",
...
var grammar = '#JSG6 V1.0; grammar boxes; public <square> = ' + boxes.join(' | ') + ' ;'
...
document.getElementById("speak").onclick=()=>{
  ...
  diagnostic.textContent='Ready to receive a square command.'
  recognition.start();
  console.log('Ready to receive a square command.');
}
...
recognition.onresult = function(event) {
  ...
  var square = event.results[0][0].transcript;
  diagnostic.textContent = 'Result received: ' + square ;
  console.log('Confidence: ' + event.results[0][0].confidence,square,event.results[0]);

  if(boxes_names.includes(square.toString().replace(".",""))){
    let fileName 
    let squareName
    let column = parseInt(square.charAt(0));
    let row=square.charAt(1);
    switch (column) {
      case 1:
        fileName = "a";
        break;
    ...
    squareName=fileName + row
    movePiece(squareName)
  }

}

recognition.onnomatch = function(event) {
  diagnostic.textContent = "I didn't recognise that square.";
}
